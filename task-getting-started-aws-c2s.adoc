...
summary: 您可以在 AWS Secret Cloud 和 Top Secret Cloud 环境中部署 Cloud Volumes ONTAP。AWS Secret Cloud 和 Top Secret Cloud 区域是特定于美国情报界的封闭区域。此页面上的说明仅适用于 AWS Secret Cloud 和 Top Secret Cloud 区域用户。 
---
...
+
要详细了解隐私模式的工作原理，请参阅 link:https://docs.netapp.com/us-en/console-setup-admin/concept-modes.html#private-mode["控制台中的私有部署模式"^]。
...
. 设置 Amazon Simple Storage Service (Amazon S3) 服务的 VPC 端点。
+
