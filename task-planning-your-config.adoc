...
NOTE: 如果您的 Cloud Volumes ONTAP 系统位于 AWS Local Zone 中，则不支持到 Amazon Simple Storage Service (Amazon S3) 的数据分层，因为在 Local Zone 之外访问 Amazon S3 存储桶涉及更高的延迟并影响 Cloud Volumes ONTAP 活动。
...
link:reference-configs-aws.html#aws["查看 AWS 中Cloud Volumes ONTAP系统数据的默认磁盘"]。
