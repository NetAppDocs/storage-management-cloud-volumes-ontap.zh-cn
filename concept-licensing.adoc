...
[role="lead"]
Cloud Volumes ONTAP 提供多种许可模式。每个选项都可让您选择满足您需求的消费模式。

您可以选择从云市场或通过 NetApp（自带许可证或 BYOL）购买或续订许可证和订阅。市场为您提供年度订阅和现收现付（PAYGO）选项。有关信息，请参阅 link:concept-license-management.html["了解有关Cloud Volumes ONTAP基于容量的许可证的更多信息"]。

BYOL 许可证可从 NetApp 购买，可用于任何云提供商。有关 BYOL 许可证可用性的详细信息，请参阅 https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/whats-new.html#restricted-availability-of-byol-licensing-for-cloud-volumes-ontap["Cloud Volumes ONTAP的 BYOL 许可可用性受限"^]。
...
新客户可以选择通过基于容量的年度市场订阅或 Keystone 订阅收费。少数部署（例如处于私有模式的部署）还具有上一代基于节点的 BYOL 许可证。
...
* link:reference-configs-azure.html["Azure 中支持的配置"]
...
* link:reference-configs-gcp.html["Google Cloud 中支持的配置"^]
...
NOTE: 备份和恢复需要按需付费（PAYGO）订阅，但使用此服务不会产生任何费用。有关设置备份和恢复许可的更多信息，请参阅 https://docs.netapp.com/us-en/data-services-backup-recovery/task-licensing-cloud-backup.html["设置备份和恢复许可"^]。
...
NOTE: NetApp 已限制 BYOL 许可的购买、延期和续期。有关详细信息，请参阅 https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/whats-new.html#restricted-availability-of-byol-licensing-for-cloud-volumes-ontap["Cloud Volumes ONTAPP的 BYOL 许可可用性受限"^]。
...
NOTE: 私有模式下的 Cloud Volumes ONTAP 私有部署。
...
|===
.异常
NetApp 认识到某些情况需要特别考虑，例如在 AWS 中的中国地区部署 Cloud Volumes ONTAP，以及基于节点的许可的 EOA 和 EOS 可能不适用于以下情况：
...
控制台可以通过许可证转换工具将基于节点的许可证无缝转换为基于容量的许可证。有关基于节点的许可的 EOA 的信息，请参阅 link:concept-licensing.html#end-of-availability-of-node-based-licenses["基于节点的许可证的可用性终止"]。
...
##如果您使用基于节点的许可证进行市场订阅或专用部署，则可以link:task-convert-node-capacity.html["将基于节点的许可证转换为基于容量的许可证"]使用 Console 中的许可证转换工具。##
...
.相关链接
* link:concept-licensing-charging.html["了解有关基于容量的许可证的更多信息"]
* link:task-manage-capacity-licenses.html["管理Cloud Volumes ONTAP基于容量的许可"]
* link:task-manage-node-licenses.html["管理基于节点的许可"]
* link:task-convert-node-capacity.html["将基于节点的许可证转换为基于容量的许可证"]
* link:task-manage-keystone.html["管理Keystone订阅"]
