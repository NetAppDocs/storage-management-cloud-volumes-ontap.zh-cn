...
Console 代理联系多个端点进行日常操作。有关使用的端点的信息，请参见 https://docs.netapp.com/us-en/console-setup-admin/task-install-connector-on-prem.html#step-3-set-up-networking["查看从控制台代理联系的端点"^]和 https://docs.netapp.com/us-en/console-setup-admin/reference-networking-saas-console.html["准备使用控制台的网络"^]。
...
在 Cloud Volumes ONTAP 部署期间，AWS mediator 代理字段接受服务器地址。如果输入完全限定的域名 (FQDN) 而不是 IP 地址，则除非使用自定义 DNS 服务器配置 AWS VPC，否则 FQDN 将无法解析。这是因为默认的 AWS 提供的 DNS 仅解析公共 DNS 记录和内部 AWS 主机，无法解析外部 FQDN，例如客户代理服务器。如果这种情况适用于您，您有两种选择：

* 直接在 mediator proxy 字段中添加代理服务器的 IP 地址，而不是 FQDN。
* 在您的 VPC 中配置 DHCP 选项集，其中包括您的自定义 DNS 服务器地址。这允许 AWS 在部署期间您在代理服务器字段中输入 FQDN 时解析该 FQDN。有关详细信息，请参阅 https://docs.aws.amazon.com/vpc/latest/userguide/DHCPOptionSet.html["AWS 文档"^]。
...
TIP: 正在寻找有关 Console 代理的信息？ https://docs.netapp.com/us-en/console-setup-admin/reference-ports-aws.html["查看控制台代理的安全组规则"^]
...
如果要将 EBS 作为性能层，将 Amazon S3 作为容量层，则必须确保 Cloud Volumes ONTAP 具有到 S3 的连接。提供此连接的最佳方法是创建到 S3 服务的 VPC 端点。有关说明，请参阅 https://docs.aws.amazon.com/AmazonVPC/latest/UserGuide/vpce-gateway.html#create-gateway-endpoint["AWS 文档：创建网关终端节点"^]。
...
* https://docs.netapp.com/us-en/console-setup-admin/concept-install-options-aws.html["查看控制台代理的网络要求"^]
