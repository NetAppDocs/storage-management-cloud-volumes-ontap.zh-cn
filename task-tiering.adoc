...
----
默认情况下，自定义角色包括权限。 https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-azure.html["查看控制台代理的 Azure 权限"^]
...
+
要获得有关复制数据的帮助，请参阅 https://docs.netapp.com/us-en/data-services-replication/task-replicating-data.html["将数据复制到云端或从云端复制数据"^]。
...
.步骤
. 从左侧导航窗格转到*管理>代理*。
. 单击管理 Cloud Volumes ONTAP 系统的 Console 代理的 image:icon-action.png["显示三个水平点的图标，用于其他操作"] 图标。
