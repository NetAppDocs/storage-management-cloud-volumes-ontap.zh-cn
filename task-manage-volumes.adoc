...
summary: NetApp Console使您能够管理卷和 CIFS 服务器。它还会提示您移动卷以避免容量问题。 
---
...
您可以在NetApp Console标准视图中管理卷，也可以通过控制台中包含的ONTAP系统管理器管理卷，以实现高级卷管理。标准视图提供了一组有限的选项来修改您的卷。系统管理器提供高级管理，例如克隆、调整大小、更改反勒索软件、分析、保护和活动跟踪的设置以及跨层移动卷。有关信息，请参阅link:task-administer-advanced-view.html["使用系统管理器管理Cloud Volumes ONTAP"]。
...
.步骤
. 选择System Manager视图以通过ONTAP System Manager调整卷的大小。请参阅 link:task-administer-advanced-view.html#get-started-with-system-manager["开始使用系统管理器"]。
. 从左侧导航菜单中，选择“存储”>“卷”。
. 从卷列表中，确定应调整大小的卷。
...
TIP: 当聚合已达到 90% 的已用容量时，控制台（Console）将显示这些"需要操作"消息。如果启用了数据分层，则在聚合达到 80% 已用容量时显示消息。默认情况下，为数据分层保留 10% 的可用空间。link:task-tiering.html#change-the-free-space-ratio-for-data-tiering["了解有关数据分层的可用空间比率的更多信息"^]。
...
.. 在聚合图块上，单击 image:icon-action.png["显示三个水平点的图标，用于其他操作"] 图标，然后单击 *查看聚合详细信息*。
...
+
有关详细信息，请参阅 link:https://docs.netapp.com/us-en/data-services-replication/task-replicating-data.html["在系统之间复制数据"^]。

. 转到“复制状态”页面，然后中断SnapMirror关系，将复制的卷从数据保护卷转换为读/写卷。
+
有关详细信息，请参阅 link:https://docs.netapp.com/us-en/data-services-replication/task-replicating-data.html#managing-data-replication-schedules-and-relationships["管理数据复制计划和关系"^]。
...
.. 在所需的聚合图块上，单击 image:icon-action.png["显示三个水平点的图标，用于其他操作"] 图标，然后单击*查看聚合详细信息*以查看可用容量（调配大小减去已用聚合容量）。
+
...
. 如果需要，将磁盘添加到现有聚合：
+
.. 选择聚合，然后单击 image:icon-action.png["显示三个水平点的图标，用于其他操作"] 图标 > *Add Disks*。
.. 选择要添加的磁盘数量，然后单击“*添加*”。
