...
WORM 功能可与您的市场订阅和 Bring-Your-Own License (BYOL) 一起使用，无需额外费用。请联系您的 NetApp 销售代表，将 WORM 添加到您当前的许可证中。
...
您可以使用应用程序将文件提交到 WORM over NFS 或 CIFS，或使用 ONTAP CLI 自动将文件提交到 WORM。您还可以使用 WORM 可追加文件来保留增量写入的数据，如日志信息。有关信息，请参阅 https://docs.netapp.com/us-en/ontap/snaplock/commit-files-worm-state-manual-task.html#commit-files-to-worm-manually["使用 ONTAP SnapLock 将文件提交到 WORM"^]。

在 Cloud Volumes ONTAP 系统上激活 WORM 存储后，必须使用 ONTAP CLI 进行 WORM 存储的所有管理。有关说明，请参见link:https://docs.netapp.com/us-en/ontap/snaplock/index.html["有关SnapLock的ONTAP文档"^]。
