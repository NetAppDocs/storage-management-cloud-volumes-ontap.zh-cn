...
[role="lead"]
Cloud Volumes ONTAP 出于两个目的需要 Google Cloud 服务帐户。第一个目的是启用 link:concept-data-tiering.html["数据分层"] 将冷数据分层到 Google Cloud 中的低成本对象存储。第二个目的是启用  https://docs.netapp.com/us-en/data-services-backup-recovery/concept-backup-to-cloud.html["NetApp Backup and Recovery"^] 将卷备份到低成本对象存储。
