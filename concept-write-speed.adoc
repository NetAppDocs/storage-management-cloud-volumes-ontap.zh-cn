...
link:reference-configs-aws.html["了解有关Cloud Volumes ONTAP支持的 Amazon EC2 实例的更多信息"]。
...
如果使用 HA 对，从 9.8 版本开始，Cloud Volumes ONTAP 支持多种 VM 类型的高写入速度。转到 link:reference-configs-azure.html["Cloud Volumes ONTAP发行说明"] 以查看支持高写入速度的 VM 类型。
...
如果使用 HA 对，从 9.13.0 版本开始，Cloud Volumes ONTAP 支持多种 VM 类型的高写入速度。转到link:reference-configs-gcp.html#supported-configurations-by-license["Cloud Volumes ONTAP发行说明"^]以查看支持高写入速度的 VM 类型。

link:reference-configs-gcp.html["详细了解Cloud Volumes ONTAP支持的 Google Cloud 机器类型"^]。
