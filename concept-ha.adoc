...
有关详细信息，请参阅 link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-networking-aws.html#requirements-for-ha-pairs-in-multiple-azs["多个可用区中Cloud Volumes ONTAP HA 的 AWS 网络要求"^]。
...
NOTE: Cloud Volumes ONTAP 仅支持这些配置。在 AWS Local Zone 配置中选择不受支持的磁盘类型或不合格的实例可能会导致部署失败。如果您的 Cloud Volumes ONTAP 系统位于 AWS Local Zone 中，则不支持将数据分层到 Amazon Simple Storage Service (Amazon S3)，因为在 Local Zone 之外访问 Amazon S3 存储桶涉及更高的延迟并影响 Cloud Volumes ONTAP 活动。
