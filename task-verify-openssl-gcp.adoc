...
summary: 使用 OpenSSL 验证 Google Cloud 下载的 disk.raw 文件是否与摘要文件内容一致。 
---
...
[role="lead"]
您可以通过以下方式验证 Google Cloud 下载的 disk.raw 文件与摘要文件内容 https://mysupport.netapp.com/site/products/all/details/cloud-volumes-ontap/downloads-tab["国家安全局"^]使用 OpenSSL。


NOTE: 用于验证图像的 OpenSSL 命令与 Linux、macOS 和 Windows 机器兼容。
...
----
====
. 将下载的 disk.raw 文件、签名和证书放在一个目录中。
. 使用 OpenSSL 从证书中提取公钥。
. 使用提取的公钥解密签名并验证下载的 disk.raw 文件的内容。
+
.点击显示
[%collapsible]
====
