...
NetApp 很高兴宣布针对私有模式 Cloud Volumes ONTAP 部署的基于容量的许可。您现在可以通过将基于节点的许可证转换为基于容量的许可来优化私有模式 Cloud Volumes ONTAP 部署的成本。对于您的私有模式部署，您可以直接从 NetApp 购买 Essentials（自带许可证 (BYOL)）软件包，其容量与您现有的基于节点的许可证相近。
