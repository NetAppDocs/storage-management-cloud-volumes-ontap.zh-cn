...
* 在您创建 iSCSI 卷时，Console 会自动为您创建一个 LUN。我们通过在每个卷上只创建一个 LUN 来简化操作，因此无需进行任何管理。在您创建卷后，link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-connect-lun.html["使用 IQN 从主机连接到 LUN"^]。
* 您可以从ONTAP系统管理器或ONTAP CLI 创建其他 LUN。


ifdef::aws[]

* 如果要在 AWS 中使用 CIFS，则必须已设置 DNS 和 Active Directory。有关详细信息，请参阅 link:reference-networking-aws.html["AWS 中Cloud Volumes ONTAP的网络要求"]。
...
.特定骨料
--
.. 在*聚合*选项卡上，转到所需的聚合，然后单击 image:icon-action.png["显示三个水平点的图标，用于其他操作"] 图标。
...
NTP 服务器:: 选择*使用 Active Directory 域*，以使用 Active Directory DNS 配置 NTP 服务器。如果需要使用其他地址配置 NTP 服务器，则应使用 API。有关信息，请参阅 https://docs.netapp.com/us-en/console-automation/index.html["NetApp Console 自动化文档"^]。
+
--
...
+
*** link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config.html#size-your-system-in-aws["在 AWS 中调整系统规模"^]
...
* link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config-azure.html#size-your-system-in-azure["在 Azure 中调整系统大小"^]
...
* link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config-gcp.html#size-your-system-in-gcp["在 Google Cloud 中调整系统规模"^]
