...
---
= 使用 Azure 中的高级选项部署 Cloud Volumes ONTAP
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
* 已启动并正在运行的控制台代理。
+
** 您应拥有 https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-azure.html["与您的系统关联的控制台代理"^]。
...
. 如果出现提示，请按 https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-azure.html["创建控制台代理"^]。
...
| 资源组标签 | 标签是 Azure 资源的元数据。当您在此字段中输入标签时，控制台会将它们添加到与Cloud Volumes ONTAP系统关联的资源组中。创建系统时，您可以从用户界面添加最多四个标签，然后可以在创建系统后添加更多标签。请注意，创建系统时，API 不会将您限制为四个标签。有关标签的信息，请参阅 https://learn.microsoft.com/en-us/azure/azure-resource-manager/management/tag-resources["Microsoft Azure 文档：使用标签来组织您的 Azure 资源"^]。 
...
| 编辑凭证 | 您可以选择不同的 Azure 凭据和不同的 Azure 订阅来与此 Cloud Volumes ONTAP 系统一起使用。您需要将 Azure Marketplace 订阅与所选 Azure 订阅关联，才能部署按需付费的 Cloud Volumes ONTAP 系统 https://docs.netapp.com/us-en/console-setup-admin/task-adding-azure-accounts.html["了解如何添加凭证"^]。 
|===
. *服务*：启用或禁用您想要或不想与Cloud Volumes ONTAP一起使用的单个服务。
+
** https://docs.netapp.com/us-en/data-services-data-classification/concept-cloud-compliance.html["了解有关NetApp Data Classification的更多信息"^]
...
TIP: 如果使用的 Azure 帐户具有 https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-azure.html["所需权限"^]，则在部署失败或删除的情况下，Console 将从资源组中删除 Cloud Volumes ONTAP 资源。
...
| 使用现有的 | 如果选择现有安全组，则它必须满足 Cloud Volumes ONTAP 要求。link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-networking-azure.html#security-group-rules["查看默认安全组"^]。 
|===
...
NOTE: 如果较新的候选发行版、一般可用性或修补程序版本可用于选定版本，则 Console 会在创建工作环境时将系统更新到该版本。例如，如果您选择 Cloud Volumes ONTAP 9.16.1 P3 且 9.16.1 P4 可用，则会发生更新。此更新不会从一个版本更新到另一个版本，例如从 9.15 到 9.16。
...
+
要获得有关选择磁盘类型和大小的帮助，请参阅 link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config-azure.html#size-your-system-in-azure["在 Azure 中调整系统大小"^]。
...
+
此选项仅适用于某些虚拟机类型。要了解支持哪些虚拟机类型，请参阅 link:reference-configs-azure.html#ha-pairs["HA 对许可证支持的配置"]。
...
+
有关更多信息，请参阅link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config-azure.html#choose-a-volume-usage-profile["了解卷使用情况"^]和link:concept-data-tiering.html["数据分层概述"^]。
...
** 磁盘大小适用于初始聚合中的所有磁盘以及使用简单配置选项时控制台创建的任何其他聚合。您可以使用高级分配选项创建使用不同磁盘大小的聚合。
+
要获得选择磁盘大小的帮助，请参阅 link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config-azure.html#size-your-system-in-azure["在 Azure 中调整系统大小"^]。
...
+
link:reference-configs-azure.html#ha-pairs["详细了解 Azure 中支持的配置"^]。
...
* link:task-deploy-cvo-azure-mktplc.html["从 Azure 市场在 Azure 中部署Cloud Volumes ONTAP"^]
