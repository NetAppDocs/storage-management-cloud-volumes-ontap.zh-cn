...
* Cloud Volumes ONTAP 的免费许可证适用于最多 500 GiB 的已调配容量，不含 NetApp 支持，且无到期日期。超过 500 GiB 的容量需要有效的许可权利，并可能产生费用。要访问完整的 Cloud Volumes ONTAP 体验和高级管理功能，请将您的 Cloud Volumes ONTAP 系统连接到控制台。通过控制台进行发现和管理需要与您的控制台帐户关联的有效 Azure Marketplace 订阅。
...
.开始之前
* 如果您计划在控制台中发现和管理已部署的系统，请确保控制台帐户与有效的 Azure Marketplace 订阅相关联。将系统发现到控制台需要有效的订阅。
