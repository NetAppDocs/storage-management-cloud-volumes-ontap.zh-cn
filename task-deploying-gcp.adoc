...
+
** 您应拥有 https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["与您的系统关联的控制台代理"^]。
** https://docs.netapp.com/us-en/console-setup-admin/concept-connectors.html["您应该准备好让控制台代理始终处于运行状态"^]。
** 与 Console 代理关联的服务帐户 https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-gcp.html["应该具有所需的权限"^]。
...
. 如果出现提示，请按 https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["创建控制台代理"^]。
...
| 服务帐户名称 | 如果您计划将 link:concept-data-tiering.html["数据分层"] 或  https://docs.netapp.com/us-en/data-services-backup-recovery/concept-backup-to-cloud.html["NetApp Backup and Recovery"^] 与 Cloud Volumes ONTAP 结合使用，则需要启用 *Service Account* 并选择具有预定义 Storage Admin 角色的服务帐户。link:task-creating-gcp-service-account.html["了解如何创建服务帐号"^]。 
...
NOTE: 这是您为 Console 设置的服务帐户 link:https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["如本页所述"^]。

单击“添加订阅”将选定的凭据与订阅关联。

要创建即用即付的 Cloud Volumes ONTAP 系统，您需要选择一个与从 Google Cloud Marketplace 订阅 Cloud Volumes ONTAP 相关联的 Google Cloud 项目。请参阅 https://docs.netapp.com/us-en/console-setup-admin/task-adding-gcp-accounts.html["将市场订阅与 Google Cloud 凭据关联"^]。
...
. *预配置的软件包*：选择其中一个软件包以快速部署 Cloud Volumes ONTAP 系统，或单击*创建我自己的配置*。预配置的软件包因所选 Cloud Volumes ONTAP 版本而异。例如，对于 Cloud Volumes ONTAP 9.18.1 及更高版本，Console 显示包含 C3 VM 的软件包，包括 Hyperdisk Balanced 磁盘。您可以根据工作负载需求修改配置，例如 IOPS 和吞吐量参数。
+
...
+
要获得选择磁盘类型和大小的帮助，请参阅 link:task-planning-your-config-gcp.html#size-your-system-in-google-cloud["在 Google Cloud 中调整系统大小"^]。

. *闪存缓存、写入速度和 WORM*：
+
.. 如果需要，启用 *Flash Cache* 或选择*普通*或*高*写入速度。
+
详细了解 link:reference-configs-gcp.html#flash-cache-support["Flash Cache"] 和 link:concept-write-speed.html["写入速度"]。

+

NOTE: 通过*高*写入速度选项可获得高写入速度和更高的最大传输单元（MTU）8,896 字节。此外，较高的 MTU 8,896 要求在部署时选择 VPC-1、VPC-2 和 VPC-3。有关 VPC-1、VPC-2 和 VPC-3 的详细信息，请参阅 https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-networking-gcp.html#requirements-for-the-console-agent["VPC-1、VPC-2 和 VPC-3 的规则"^]。
...
** 要获得有关添加 Google Cloud 帐户的帮助，请参阅  https://docs.netapp.com/us-en/console-setup-admin/task-adding-gcp-accounts.html["使用 9.6 设置和添加 Google Cloud 帐户以进行数据分层"^]。
...
| NTP 服务器 | 选择 *Use Active Directory Domain* 可使用 Active Directory DNS 配置 NTP 服务器。如果需要使用其他地址配置 NTP 服务器，则应使用 API。有关详细信息，请参阅 https://docs.netapp.com/us-en/console-automation/index.html["NetApp Console自动化文档"^]。请注意，您只能在创建 CIFS 服务器时配置 NTP 服务器。创建 CIFS 服务器后，无法对其进行配置。 
|===
...
. *系统详细信息*：选择项目，指定集群名称，可选择服务帐户，可选择添加标签，然后指定凭据。
+
下表描述了您可能需要指导的字段：
...
| Google Cloud 项目和 Marketplace 订阅  a| 
选择要用于此系统的 Google 项目和订阅。这是您为 Console 设置的服务帐户。有关信息，请参阅link:https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["Console 设置文档"^]。Console 使用该项目创建 Cloud Volumes ONTAP 系统，并使用订阅确定计费和使用情况。如果您在下拉列表中未看到任何其他项目，则表示您尚未将服务帐户与其他项目关联。转到 Google Cloud Console，打开 IAM 服务，然后选择项目。将具有您用于 Console 的角色的服务帐户添加到该项目。您需要为每个项目重复此步骤。

单击“添加订阅”将选定的凭据与订阅关联。

要创建即用即付的 Cloud Volumes ONTAP 系统，您需要选择一个与从 Google Cloud Marketplace 订阅 Cloud Volumes ONTAP 相关联的 Google Cloud 项目。请参阅 Console 设置文档： https://docs.netapp.com/us-en/console-setup-admin/task-adding-gcp-accounts.html["将市场订阅与 Google Cloud 凭据关联"^]。
...
| 服务帐户名称 | 如果您计划使用link:concept-data-tiering.html["NetApp Cloud Tiering"]或 https://docs.netapp.com/us-en/data-services-backup-recovery/concept-backup-to-cloud.html["备份和恢复"^]服务，则需要启用 *Service Account* 开关，然后选择具有预定义 Storage Admin 角色的服务帐户。 


| 标签 | 标签是您的 Google Cloud 资源的元数据。控制台将标签添加到Cloud Volumes ONTAP系统以及与该系统关联的 Google Cloud 资源。创建系统时，您可以从用户界面添加最多四个标签，然后可以在创建系统后添加更多标签。请注意，创建系统时，API 不会将您限制为四个标签。有关标签的信息，请参阅 https://cloud.google.com/compute/docs/labeling-resources["Google Cloud 文档：标记资源"^]。 


| ONTAP 用户名和密码 | 这些是Cloud Volumes ONTAP集群管理员帐户的凭据。您可以使用这些凭据通过ONTAP System Manager 或ONTAP CLI 连接到Cloud Volumes ONTAP 。保留默认的_admin_用户名或将其更改为自定义用户名。 
|===
...
. *部署拓扑*：为 HA 配置选择多个可用区 (AZ)（推荐）或单个可用区。
+
link:concept-ha-google-cloud.html["了解有关 HA 部署模型的更多信息"^] 。

. *网络配置和连接*：选择相关区域和可用区。为 HA 配置选择四个不同的 VPC，在每个 VPC 中选择一个子网，然后选择防火墙策略。
+

NOTE: Cloud Volumes ONTAP 9.19.1 及更高版本支持link:concept-ha-google-cloud.html["单 AZ HA 配置的共享磁盘"]，使 Mediator 实例成为可选。在此配置中，您无需输入中介区域、相应的 VPC 和子网即可进行数据复制。
...
+
如果您选择其中一个程序包，则只需指定一个卷，然后查看并批准配置。预配置的程序包因所选 Cloud Volumes ONTAP 版本和配置类型而异。例如，如果您为 Cloud Volumes ONTAP 9.19.1 或更高版本选择单 AZ 部署，则可以看到带有 C3 系列计算机和 Hyperdisk Balanced 磁盘的预配置程序包。对于早期版本和多个 AZ，您仍然可以选择 n2-standard 计算机和 Persistent Disks。您可以根据工作负载需求修改配置，例如 IOPS 和吞吐量参数。
...
NOTE: 通过 n2-standard-16、n2-standard-32、n2-standard-48 和 n2-standard-64 实例类型的 *High* 写入速度选项，可以获得高写入速度和更高的最大传输单元 (MTU)（8,896 字节）。此外，较高的 MTU（8,896）需要选择 VPC-1、VPC-2 和 VPC-3 进行部署。高写入速度和 8,896 的 MTU 取决于功能，不能在已配置的实例中单独禁用。有关 VPC-1、VPC-2 和 VPC-3 的详细信息，请参阅  https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-networking-gcp.html#requirements-for-the-connector["VPC-1、VPC-2 和 VPC-3 的规则"^]。
...
| NTP 服务器 | 选择*Use Active Directory Domain*可使用 Active Directory DNS 配置 NTP 服务器。如果需要使用其他地址配置 NTP 服务器，则应使用 API。请参阅 https://docs.netapp.com/us-en/console-automation/index.html["NetApp Console自动化文档"^]了解详细信息。请注意，您只能在创建 CIFS 服务器时配置 NTP 服务器。创建 CIFS 服务器后，无法对其进行配置。 
|===
. *使用情况配置文件、磁盘类型和分层策略*：选择是否要启用存储效率功能并更改卷分层策略（如果需要）。
+
更多信息，请参阅link:task-planning-your-config-gcp.html#choose-a-volume-usage-profile["选择卷使用情况配置文件"^]，link:concept-data-tiering.html["数据分层概述"^] ， 和 https://kb.netapp.com/Cloud/Cloud_Volumes_ONTAP/What_Inline_Storage_Efficiency_features_are_supported_with_CVO#["KB：CVO 支持哪些内联存储效率功能？"^]
