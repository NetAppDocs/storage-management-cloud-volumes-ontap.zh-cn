...
机器类型:: 在 link:reference-configs-gcp.html["Cloud Volumes ONTAP发行说明"^] 中查看受支持的机器类型，然后查看 Google 提供的有关每种受支持机器类型的详细信息。将您的工作负载要求与机器类型的 vCPU 和内存数量相匹配。请注意，每个 CPU 核心都会提高网络性能。
+
--
请参阅以下内容以了解更多详细信息：
...
* link:reference-configs-gcp.html#google-cloud-single-node["查看 Google Cloud 中Cloud Volumes ONTAP系统数据的默认磁盘"]。
...
TIP: Console 代理还需要系统磁盘。 https://docs.netapp.com/us-en/console-setup-admin/reference-connector-default-config.html["查看控制台代理默认配置的详细信息"^]。
