...
---
= 使用 AWS 中的高级选项部署 Cloud Volumes ONTAP
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
* 已启动并正在运行的控制台代理。
+
** 您应拥有 https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-aws.html["与您的系统关联的控制台代理"^]。
...
. 如果系统提示您， https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-aws.html["创建控制台代理"^]。
...
| 编辑凭证 | 选择与要部署此系统的帐户关联的 AWS 凭据。您还可以将 AWS 市场订阅与此 Cloud Volumes ONTAP 系统关联使用。单击 *添加订阅* ，将所选凭据与新的 AWS 市场订阅关联。订阅可以是年度合同，也可以是按小时费率为 Cloud Volumes ONTAP 付费。https://docs.netapp.com/us-en/console-setup-admin/task-adding-aws-accounts.html["了解如何向NetApp Console添加其他 AWS 凭证"^]。 
|===
+
...
. *位置和连接* ：输入您在 https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config.html#collect-networking-information["AWS 工作表"^]中记录的网络信息。
+
下表描述了您可能需要指导的字段：
...
| 编辑凭证 | 选择要与此 Cloud Volumes ONTAP 系统一起使用的 AWS 凭据和市场订阅。单击*添加订阅*，将所选凭据与新的 AWS 市场订阅相关联。订阅可以是年度合同，也可以是按小时费率支付 Cloud Volumes ONTAP。如果您直接从 NetApp（Bring-Your-Own License 或 BYOL）购买了许可证，则不需要 AWS 订阅。NetApp 已限制 BYOL 许可的购买、扩展和续订。有关详细信息，请参阅 https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/whats-new.html#restricted-availability-of-byol-licensing-for-cloud-volumes-ontap["Cloud Volumes ONTAP的 BYOL 许可可用性受限"^]。https://docs.netapp.com/us-en/console-setup-admin/task-adding-aws-accounts.html["了解如何向控制台添加其他 AWS 凭证"^]。 
|===
+
...
+
** https://docs.netapp.com/us-en/data-services-data-classification/concept-cloud-compliance.html["了解有关NetApp Data Classification的更多信息"^]
** https://docs.netapp.com/us-en/data-services-backup-recovery/task-backup-to-s3.html["了解有关备份和恢复的更多信息"^]
+
...
+
IP 地址必须在该区域中所有 VPC 的 CIDR 块范围之外。有关其他详细信息，请参阅 link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-networking-aws.html#requirements-for-ha-pairs-in-multiple-azs["多个可用区中Cloud Volumes ONTAP HA 的 AWS 网络要求"^]。
...
. *使用情况配置文件、磁盘类型和分层策略*：选择是否要启用存储效率功能，并在需要时编辑卷分层策略。
+
有关更多信息，请参阅link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config.html#choose-a-volume-usage-profile["选择卷使用情况配置文件"^]和link:concept-data-tiering.html["数据分层概述"^]。
...
.相关链接
* link:task-onboard-cvo-aws-marketplace.html["从 AWS Marketplace 订阅和部署 Cloud Volumes ONTAP"]
