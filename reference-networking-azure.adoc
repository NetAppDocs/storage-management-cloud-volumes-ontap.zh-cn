...
Console 代理还会联系多个端点进行日常操作。有关端点的信息，请参见 https://docs.netapp.com/us-en/console-setup-admin/task-install-connector-on-prem.html#step-3-set-up-networking["查看从控制台代理联系的端点"^]和 https://docs.netapp.com/us-en/console-setup-admin/reference-networking-saas-console.html["准备使用控制台的网络"^]。
...
有关配置代理服务器的信息，请参见 https://docs.netapp.com/us-en/console-setup-admin/task-configuring-proxy.html["配置控制台代理以使用代理服务器"^]。
...
Cloud Volumes ONTAP 支持 https://learn.microsoft.com/en-us/azure/virtual-network/virtual-network-encryption-overview["Azure 虚拟网络 (VNet) 加密"^]对 VNet 内部或跨对等 VNet 的 VM 到 VM 流量进行加密。此功能在 Azure VNet 层配置，独立于 Cloud Volumes ONTAP 拓扑（单节点或 HA）。

只需确保在虚拟机的 NIC 上启用加速网络，并在启用该功能之前查看 Azure VNet 加密要求和限制即可。不应修改 NetApp 托管负载平衡器对象。

https://learn.microsoft.com/en-us/azure/virtual-network/accelerated-networking-overview?tabs=NetworkManager["Azure 文档：VNet 加密和 Accelerated Networking"^]。
...
控制台创建 Azure 安全组，其中包括 Cloud Volumes ONTAP 成功运行所需的入站和出站规则 https://docs.netapp.com/us-en/console-setup-admin/reference-ports-azure.html["查看控制台代理的安全组规则"^]。
...
我们不建议修改预定义的安全组或使用自定义安全组。但是，如果必须，请注意，部署过程需要 Cloud Volumes ONTAP 系统在其自己的子网内具有完全访问权限。部署完成后，如果您决定修改网络安全组，请确保集群端口和 HA 网络端口保持打开状态，因为 HA 网络依赖于两个节点之间不受限制的通信。此外，节点的 IP 地址在部署过程中会自动定义。这确保了 Cloud Volumes ONTAP 集群内的无缝通信（节点之间的任意通信）。
...
NOTE: HA 系统的入站规则少于单节点系统，因为入站数据流量通过 Azure Standard Load Balancer。因此，应打开来自 Load Balancer 的流量，如"AllowAzureLoadBalancerInBound"规则中所示。
...
* https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-azure.html["查看控制台代理的网络要求"^]
