...
* 如果聚合达到容量阈值且无法支持任何额外的磁盘，Console 会自动将卷从该聚合移动到具有可用容量的聚合或新聚合。
+
如果 Console 为卷创建新的聚合，则会选择适合该卷大小的磁盘大小。

+
请注意，原始聚合上的可用空间现在可用。现有卷或新卷可以使用该空间。在此方案中，空间无法返回到云提供商。
...
* 要了解 AWS 中的信息配置，请参阅 link:reference-configs-aws.html["AWS 中聚合的磁盘配置"]。
