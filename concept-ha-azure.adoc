...
image:diagram_ha_azure.png["概念图显示了一个负载均衡器管理来自NetApp Console和客户端的传入流量、VMSS Flex 中的两个Cloud Volumes ONTAP节点、用于启动数据的 SSD 磁盘以及用于客户数据的 Page Blob。"]
...
磁盘:: 客户数据驻留在 Premium Storage 页 blob 上。每个节点都可以访问另一个节点的存储。还需要额外的存储用于 link:reference-configs-azure.html#azure-ha-pair["引导、根和核心数据"]。
存储帐户::
+
--
* 托管磁盘需要一个存储帐户。
...
NOTE: 从 Cloud Volumes ONTAP 9.15.0P1 开始，新的高可用性对部署不再支持 Azure 页面 Blob。如果当前在现有的高可用性对部署中使用 Azure 页面 Blob，则可以迁移到 Edsv4 系列虚拟机和 Edsv5 系列虚拟机中较新的虚拟机实例类型。link:reference-configs-azure.html#ha-pairs["详细了解 Azure 中支持的配置"]。
...
NOTE: 如果您为符合上述条件的环境选择高级 SSD 管理磁盘，控制台将自动部署高级 SSD v2 管理磁盘。您无法切换到高级 SSD v1 管理磁盘。

磁盘:: 客户数据驻留在本地冗余存储 (LRS) 托管磁盘上。每个节点都可以访问另一个节点的存储。还需要额外的存储用于 link:reference-configs-azure.html#azure-ha-pair["启动、根、合作伙伴根、核心和NVRAM数据"]。
...
磁盘:: 客户数据驻留在区域冗余存储 (ZRS) 托管磁盘上。每个节点都可以访问另一个节点的存储。还必须为 link:reference-configs-azure.html#azure-ha-pair["启动、根、合作伙伴根和核心数据"] 提供额外的存储。
存储帐户:: 存储帐户用于基于托管磁盘的部署，以处理诊断日志和分层到 Blob 存储。
...
NOTE: Cloud Volumes ONTAP 在主动-被动模式下使用负载均衡器进行探测。在 Azure 中，只有活动节点响应探测。被动节点可能显示为非活动状态，并带有 X 标记。这是预期行为，无需担心。

.相关链接
link:task-azure-high-availability-mode.html["在 Azure 中为Cloud Volumes ONTAP启用高可用性模式"^]
