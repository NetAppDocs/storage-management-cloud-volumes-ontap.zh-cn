...
summary: 设置您的 Google Cloud 网络，以便 Cloud Volumes ONTAP 系统可以正常运行。您将了解 CVO 的网络要求、私有 IP 地址、VPC、子网、数据包镜像、防火墙规则和 HA 配置以及控制台代理要求。 
---
...
https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["查看控制台代理部署中涵盖的所需共享 VPC 权限"^]
...
Console 代理还会联系多个端点以执行日常操作。有关端点的信息，请参见 https://docs.netapp.com/us-en/console-setup-admin/task-install-connector-on-prem.html#step-3-set-up-networking["查看从控制台代理联系的端点"^]和 https://docs.netapp.com/us-en/console-setup-admin/reference-networking-saas-console.html["准备使用控制台的网络"^]。
...
TIP: 正在寻找有关 Console 代理的信息？ https://docs.netapp.com/us-en/console-setup-admin/reference-ports-gcp.html["查看控制台代理的防火墙规则"^]
...
=== 私有模式或受保护环境中的 Infrastructure Manager 配置

如果要在私有模式或受 VPC Service Controls 保护的项目中部署 Cloud Volumes ONTAP 9.16.1 或更高版本，您需要进行一些配置更改，以便 Cloud Volumes ONTAP 可以使用 Google Cloud Infrastructure Manager 作为部署服务，而不是 Deployment Manager（Google 最终将弃用该服务）。

.开始之前
* 确保您的 Cloud Volumes ONTAP 系统为 9.16.1 或更高版本。如果不是，请升级您的系统。有关说明，请参阅 https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-updating-ontap-cloud.html["升级Cloud Volumes ONTAP"]。
* 请确保已启用 Google Cloud API。请参阅 https://docs.netapp.com/us-en/console-setup-admin/task-install-agent-google-gcloud.html#step-5-enable-google-cloud-apis["启用 Google Cloud API"^]。
* 确保已启用 Cloud Build API。请参阅 https://console.cloud.google.com/flows/enableapi?apiid=cloudbuild.googleapis.com&redirect=https://console.cloud.google.com/build/docs/private-pools/set-up-private-pool-environment["在此处启用 Cloud Build API"^]。
* 验证 Console 代理的服务帐户是否具有所有标准权限。此外，请确保服务帐户具有 `cloudbuild.workerpools.get`和 `cloudbuild.workerpools.list`权限。请参阅 https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-gcp.html#service-account-permissions["控制台代理的 Google Cloud 权限"^]。


.步骤
. 在与 Cloud Volumes ONTAP 部署相同的区域中使用此配置创建专用工作者池。有关创建专用工作者池的信息，请参阅 https://docs.cloud.google.com/build/docs/private-pools/create-manage-private-pools#creating_a_new_private_pool["Google Cloud 文档：创建和管理私有池"^]和 https://cloud.google.com/build/pricing["Google Cloud Build 定价"^]。
+
如果在受 VPC Service Controls 保护的项目中部署 Cloud Volumes ONTAP，请在与 Console agent 相同的项目中创建私有工作线程池。工作线程池必须处于运行状态。Google Cloud 的默认公共 Cloud Build 工作线程池无法在服务边界内运行。
...
** 配置为访问 https://cloud.google.com/apis["Google Cloud API"^]的子网。执行以下步骤以确保子网可以访问 Google API：
+
... 确保子网的 "Private Google Access" 已打开。
... 转到 *VPC Network level > Private Service Access Tab > Allocated IP ranges for services*。
... 选择 *分配 IP 范围* ，并为与 Google Compute Service 的私有连接分配内部 IP 范围。
... 在 *Private connection to services* 上，选择 *Create Connection*。
... 选择 *Connected service producer = Google Cloud Platform*。
... 为您在上一步中创建的专用连接 IP 范围分配配额。




. 部署此工作者池并使其运行以进行 Cloud Volumes ONTAP 管理。Google Cloud 使用此工作者池在隔离环境中运行所有 Terraform 操作。
. 在私有模式下部署 Cloud Volumes ONTAP 时，请在 *GCP Worker Pool* 字段中选择此工作池的名称。有关说明，请参阅 link:task-deploying-gcp.html["在 Google Cloud 中启动Cloud Volumes ONTAP"]。
+
如果在受 VPC Service Controls 保护的项目中部署 Cloud Volumes ONTAP，请使用 API 创建 Cloud Volumes ONTAP 系统，并在创建请求正文中包含 `workerPool`参数。

+

NOTE: 当前不支持在使用控制台 UI 部署 Cloud Volumes ONTAP 时包含工作池。对于受 VPC Service Controls 保护的项目，API 是部署 Cloud Volumes ONTAP 的唯一受支持方式。
...
* https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["查看控制台代理的网络要求"^]
...
有关为控制台代理配置代理服务器的信息，请参见 https://docs.netapp.com/us-en/console-setup-admin/task-configuring-proxy.html["配置控制台代理以使用代理服务器"^]。
